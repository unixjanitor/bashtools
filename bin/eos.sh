...
# eos.sh - Execute Over SSH
# Version 2.0.1
#   - Fixed bug with spaces in path names
# Distributed command script for large environments
# Jeff Hudson
...
get_cmdline ()
{
	# If $password is set, send the ssh connection through expect
	if [ "${password}" == "true" ]; then
		cmd_str=expect\ -c\ \"spawn\ ssh\ \$\{user\}\@\$\{host\}\ \$\{command\}\;expect\ password\:\ \;send\ \$\{mypassword\}\\n\;\ interact\ \"\ \2\>\&1\|grep\ -v\ -e\ \"\$\{user\}\@\$\{host\}\'s\ password\:\"\ -e\ \"spawn\ ssh\ -o\ User\=\$\{user\}\ -o\ IdentityFile\=\$\{my_key\}\ \$\{host\}\ \$\{command\}\"\ -e\ \"\^send:\"\ -e\ \"'\"'send\ \$\{mypassword\}\"\ -e\ \"while\ executing\"
	else
		cmd_str=ssh\ -o\ User\=\$\{user\}\ -o\ IdentityFile\=\$\{my_key\}\ \$\{host\}\ \$\{\command\}
	fi
}
...
print_help ()
{
	echo -e "${bold_on}Usage:${bold_off}\n\t${0} [-H] [-u USER] [-k IDENTITY_KEY_FILE] [-h HOST|HOST_GROUP|HOST_FILE] [-s SLEEP_SEC(R for Random)] [-P] [-p [PASSWORD]] [-e ECHOPADDING] [-i] [-d] [-r] [-c] ${bold_on}COMMAND${bold_off}\n"
...
# Get the options
while getopts ":u:h:c:e:k:s:Pp:Hrid" opt
do
	case "${opt}" in
		H)
